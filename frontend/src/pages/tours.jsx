    ...
    <Layout title="Tour">
      ...
      <div className="bg-white">
        <Section className="max-w-7xl mx-auto">
          ...
          <div className="mt-16 h-[30rem]">{toursRes && <CardStack data={toursRes.filter((tour) => tour.tourType === 'cultural')} />}</div>
        </Section>
      </div>
      <div className="bg-gray-100 ">
        <Section className="max-w-7xl mx-auto ">
          <div className=" flex flex-col items-center pt-12">
            <h1 className="text-center text-3xl font-bold">Sri Lankan Adventure Tours</h1>
            <p className="text-center w-[70%] text-sm">
              Get your adrenaline pumping with our Sri Lankan adventure tour. Trek through lush rainforests, raft down raging rivers, and climb towering mountains. This tour is perfect for thrill-seekers who want to experience the best of Sri Lanka's natural beauty.
            </p>
          </div>
          <div className="mt-16 h-[30rem]">{toursRes && <CardStack data={toursRes.filter((tour) => tour.tourType === 'adventure')} />}</div>
        </Section>
      </div>
      <div className="bg-white">
        <Section className="max-w-7xl mx-auto ">
          <div className=" flex flex-col items-center pt-12">
            <h1 className="text-center text-3xl font-bold">Sri Lankan Beach Escapes</h1>
            <p className="text-center w-[70%] text-sm">Escape to paradise on our Sri Lankan beach tour. Relax on pristine white-sand beaches, swim in crystal-clear waters, and soak up the sun. This tour is perfect for those who want to unwind and enjoy the beauty of Sri Lanka's coastline.</p>
          </div>
          <div className="mt-16 h-[30rem]">{toursRes && <CardStack data={toursRes.filter((tour) => tour.tourType === 'beach')} />}</div>
        </Section>
      </div>
      <div className="bg-gray-100">
        <Section className="max-w-7xl mx-auto ">
          <div className=" flex flex-col items-center pt-12">
            <h1 className="text-center text-3xl font-bold">Sri Lankan Camping Adventures</h1>
            <p className="text-center w-[70%] text-sm">
          ...
          <div className="mt-16 h-[30rem]">{toursRes && <CardStack data={toursRes.filter((tour) => tour.tourType === 'camping')} />}</div>
        </Section>
      </div>
    </Layout>
  )
