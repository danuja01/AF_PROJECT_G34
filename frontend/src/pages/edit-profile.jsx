...
export function EditProfile() {
    ...
    useEffect(() => {
        axios.get(`http://localhost:4000/api/users/${id}`).then((res) => {
            ...
            setCurrentID(res.data.users._id)
    ...
    const sendData = (e) => {
        ...
        axios.patch("http://localhost:4000/api/users/", updatedUser).then(() => {
            alert("User Updated");
            navigate('/home/')
        }).catch((err) => {
            alert(err)
                    ...
                    <center>
                        <div>
                            <form onSubmit={formHandler}>
                                ...
                                <button type="submit" className="flex-shrink-0 bg-teal-500 hover:bg-teal-700 border-teal-500 hover:border-teal-700 text-sm border-4 text-white py-2 px-3 rounded">Upload Photo</button>
                                Uploading done {progress}%
                            </form>
                        </div>
                        <form class="w-full max-w-lg" onSubmit={sendData}>
